...
class Model1d_eloi(nn.Module):
    def __init__(
        ...
        UNet = UNetConditional1d if use_classifier_free_guidance else UNet1d_eloi
...
class Model1d(nn.Module):
    def __init__(
        self, diffusion_type: str='k', use_classifier_free_guidance: bool = False, **kwargs
    ):
        super().__init__()
        diffusion_kwargs, kwargs = groupby("diffusion_", kwargs)

        if use_classifier_free_guidance:
            self.unet = UNetConditional1d(**kwargs, embedding_mask_proba=self.embedding_mask_proba)
        else:
...
def get_default_model_kwargs():
    return dict(
        ...
        diffusion_latent=True,
        embedding_max_length=1280,
        embedding_features=128,
)


def get_default_sampling_kwargs():
    return dict(sigma_schedule=KarrasSchedule, sampler=VSampler(), clamp=False)
