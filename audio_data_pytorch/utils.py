...
def fast_scandir(path: str, exts: List[str], recursive: bool = False, instruments: str = ""):
    # Scan files recursively faster than glob
    # From github.com/drscotthawley/aeiou/blob/main/aeiou/core.py
    subfolders, files = [], []  
    instruments_lower = [instrument.lower() for instrument in instruments]
            ...
            try:  # 'hope to avoid too many levels of symbolic links' error
                if f.is_dir():
                    ...
                    if os.path.splitext(f.name)[1].lower() in exts:
                        if not instruments or any(instr in f.name.lower() for instr in instruments_lower):
                            files.append(f.path)
            except Exception:
                pass
    except Exception:
    ...
    if recursive:
        for path in list(subfolders):
            sf, f = fast_scandir(path, exts, recursive=recursive, instruments=instruments)
